...
Chatwoot(fork, 별도 레포)와 나란히 동작하는 팀 자체 신규 개발 3종을 모아둔 레포입니다.
...
├── webhook-bot/  Agent Bot webhook 수신 서버 (FastAPI) — 자동응답/핸드오프
└── docker-compose.yml
​```
...
| `webhook-bot/` | 나머지 1명 | [`webhook-bot/README.md`](./webhook-bot/README.md) |
...
1. `docker compose up`으로 webhook-bot 컨테이너 기동 확인 (`GET /health`)
2. Chatwoot 대시보드 → Settings → Bots → Add Bot
3. Webhook URL에 **`http://webhook-bot:8001/webhook/agent-bot`** 입력 (`localhost` 금지)
4. 발급된 secret을 `webhook-bot/.env`의 `AGENT_BOT_WEBHOOK_SECRET`에 저장
5. Settings → Inboxes → Bot Configuration에서 봇 연결
...
- webhook-bot: LLM 연동은 4~5주차 작업 (현재 키워드 매칭 PoC)
